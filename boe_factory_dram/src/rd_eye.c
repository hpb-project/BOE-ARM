...
void set_rd_dqs_delay(int position, int lane) {
//	int i;

	//for(i=0; i<ddr_config__lanes; i++) {
		*(volatile unsigned int *)(UINTPTR)(LANE0LCDLR3_OFFSET + (LANEOFFSET*lane)) = ((rd_center[lane].qsd) + position);
		asm("dsb sy");
		*(volatile unsigned int *)(UINTPTR)(LANE0LCDLR4_OFFSET + (LANEOFFSET*lane)) = ((rd_center[lane].qsnd) + position);
		asm("dsb sy");
	//}
...
void measure_rd_eye(unsigned long int testaddr, unsigned int len, unsigned int pattern, unsigned int num_iters) {
	...
	for (lane=0; lane<ddr_config__lanes; lane++) {
	for (j=0; j<num_iters; j++) {
		...
		while(!done) {
			...
			set_rd_dqs_delay(position,lane);

// add delay 20 ctl_clk, add sleep.h and sleep for 1ms 
	      		usleep(1);
