...
{
	int Status = FlashRead(&(handle->flashInstance), handle->current_addr, sizeof(EnvContent), env);
	if(Status != XST_SUCCESS){
		printf("Flash read failed.\n");
		return XST_FAILURE;
	}
...
static void dumpEnv(EnvContent *env)
{
	printf("env->start_magic = 0x%x.\n", env->start_magic);
	printf("env->end_magic = 0x%x.\n", env->end_magic);
	printf("env->seq_id = 0x%x.\n", env->seq_id);
	printf("env->envChecksum = 0x%x.\n", env->envChecksum);
}

static int writeEnv(EnvHandle *handle, EnvContent *env)
{
	u32 writelen = sizeof(EnvContent);
	env->seq_id++;
...
void initDefEnv(EnvContent *env)
{
